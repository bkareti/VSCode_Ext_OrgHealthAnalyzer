import type { AgentforceInsightsReport, ReadinessPackAssessment } from '@/types';

/**
 * Client-side fallback for the AI Insights panel, used only until the
 * extension host has computed `agentforceInsights` on the AnalysisResult (or
 * for a stale/legacy cached result missing the field). Mirrors the shape and
 * logic of `buildAgentforceInsightsBase` in src/services/agentforceInsights.ts —
 * duplicated here because the webview bundle cannot import extension-host
 * services, only shared types.
 */
export function buildAgentforceInsightsFallback(pack: ReadinessPackAssessment): AgentforceInsightsReport {
  const insights: AgentforceInsightsReport['insights'] = [];
