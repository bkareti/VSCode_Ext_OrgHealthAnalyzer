...
import { readinessStatusCounts, projectedScoreAfterFullRemediation } from '../derivations';
  ...
  return (
    <GlassCard title="Overall Readiness Breakdown">
      ...
      <p className="text-[11px] text-sf-muted mt-3 pt-2 border-t border-sf-border/50">
        Fix the {pack.blockingIssues.length} critical gap{pack.blockingIssues.length === 1 ? '' : 's'} to improve
        readiness from {pack.overallScore}% to an estimated {projected}%.
      </p>
    </GlassCard>
  );
}
