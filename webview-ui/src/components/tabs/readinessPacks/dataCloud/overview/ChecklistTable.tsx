...
import { checklistTriState } from '../derivations';
...
export default function ChecklistTable({ pack }: Props) {
  ...
  const visible = showAll ? signals : signals.slice(0, VISIBLE_COUNT);

  return (
    <GlassCard title="Data Cloud Readiness Checklist">
      <div className="overflow-x-auto">
        <table className="w-full text-left" style={{ minWidth: 720 }}>
          <thead>
            <tr>
              {['Requirement', 'Available', 'Configured', 'In Use', 'Status', 'Remediation'].map((h) => (
            ...
            {visible.map((signal, i) => {
              ...
              return (
                <tr key={signal.id} className={i % 2 ? 'bg-sf-glass-stripe' : ''}>
                  ...
                  <td className="px-2 py-2 text-[11px] text-sf-muted align-top" style={{ maxWidth: 280 }}>
      ...
      <div className="flex items-center justify-between mt-2">
        {signals.length > VISIBLE_COUNT ? (
          <button className="text-xs text-sf-accent hover:underline" onClick={() => setShowAll((v) => !v)}>
            {showAll ? '▴ Show fewer' : `View full checklist (${signals.length}) →`}
          </button>
        ) : <span />}
