...
import { PerfLimitsSummary } from './perfLimitsSummary';

// Anchored dropdown for the header's Scan History / Filters actions — mirrors
// the same local helper in GovernorLimits.tsx/SecurityAccess.tsx (no shared
// popover component exists yet).
  ...
  if (!results) {
    return (
      <EmptyState
        ...
        description="Run a full analysis to see architecture, code quality, runtime performance, and scalability metrics."
        className="m-6"
      />
    );
  }
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-base font-semibold text-sf-text mb-1 flex items-center gap-1.5">
            ...
            <Tooltip content="Architecture-derived scores are always available; operational and runtime sections light up as debug logs and Shield Event Monitoring data become available.">
              <span className="text-[11px] text-sf-muted cursor-help">ⓘ</span>
            </Tooltip>
          </h1>
          <p className="text-xs text-sf-muted">
            Comprehensive analysis of your org's architecture, code quality, runtime performance and scalability.
          </p>
        </div>
            ...
            {historyNewestFirst.length > 0 ? (
              <div className="space-y-1.5 max-h-64 overflow-y-auto">
                {historyNewestFirst.map((h, i) => {
                  ...
                  const delta = prev ? Math.round((h.scores.performance ?? 0) - (prev.scores.performance ?? 0)) : undefined;
                  return (
                    <div key={h.timestamp} className="flex items-center justify-between text-[11px] py-1 border-b border-sf-border/40 last:border-0">
                      ...
                      <span className="flex items-center gap-1.5">
                        <span className="font-semibold text-sf-text tabular-nums">{Math.round(h.scores.performance ?? 0)}</span>
                        {delta !== undefined && (
